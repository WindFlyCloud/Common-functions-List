...
@interface CXPopTableMenu ()<UITableViewDataSource, UITableViewDelegate>
...
@property (nonatomic, assign) CXPopMenuDirection direction;

@end
@implementation CXPopTableMenu
...
- (instancetype)initWithDataArray:(NSArray *)dataArray
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)]) {
        ...
        if (direction == kCXPopMenuDirectionLeft) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //取出模型
    CXCellModel *model = [self.dataArray objectAtIndex:indexPath.row];
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.direction == kCXPopMenuDirectionLeft) {
...
#pragma mark - CXCellModel

@implementation CXCellModel

- (instancetype)initWithTitle:(NSString *)title imageName:(NSString *)imageName {
    CXCellModel *model = [[CXCellModel alloc] init];
    model.title = title;
